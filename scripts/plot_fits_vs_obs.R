...
# Helper function to create summary point plot
create_summary_plot <- function(data, metric, xlabel, title, x_limit = NULL) {
  summary_data <- data %>%
    group_by(model) %>%
    summarise(
  ...
  if (is.null(x_limit)) {
    x_limit <- max(summary_data$mean + summary_data$sd) * 1.2
  }

  ggplot(summary_data, aes(y = model, x = mean, color = model)) +
    geom_point(size = 3) +
    geom_errorbarh(aes(xmin = mean - sd, xmax = mean + sd), height = 0.3) +
    scale_color_manual(values = rev(COLOR_PALETTE[-1])) +
    scale_y_discrete(labels = rev(MODEL_LABELS)) +
    labs(y = NULL, x = xlabel, title = title) +
    geom_text(aes(label = sprintf("%.2f ± %.2f", mean, sd)),
              hjust = 0.5, vjust = -1,  size = ANNOTATION_SIZE, color = "black") +
    theme_cowplot(font_size = 8) +
    theme(legend.position = "none",
          plot.title = element_text(face = "plain", color = "#4a4a4a")) +
    scale_x_continuous(
      limits = c(0, x_limit),
      expand = expansion(mult = c(0.1, 0), add = c(0, 0))
    )
}

# Helper function to create best model histogram
create_best_model_histogram <- function(data) {
  count_data <- data %>%
    count(best_model) %>%
    mutate(best_model = factor(best_model, levels = rev(MODEL_NAMES)))

  ggplot(count_data, aes(y = best_model, x = n, fill = best_model)) +
    geom_col() +
    scale_fill_manual(values = rev(COLOR_PALETTE[-1])) +
    scale_y_discrete(labels = rev(MODEL_LABELS)) +
    labs(y = NULL, x = "Count", title = "Best model") +
    geom_text(aes(label = n), hjust = -0.2, size = ANNOTATION_SIZE, color = "black") +
    theme_cowplot(font_size = 8) +
    theme(legend.position = "none",
          plot.title = element_text(face = "plain", color = "#4a4a4a")
          ) +
...
create_faceted_p2 <- function(plot_data, gof_metrics, y_limits) {
  ...
  ggplot(long_data, aes(x = nlrmsf)) +
    ...
    geom_label(data = gof_metrics,
               aes(x = -Inf, y = Inf,
                   label = sprintf("RMSE(flex): %.2f", rmse_lrmsf)),
               hjust = -0.1, vjust = 1.1, size = ANNOTATION_SIZE,
               label.padding = unit(0.15, "lines"),
               label.r = unit(0, "lines"),
               fill = "white",
...
create_faceted_p3 <- function(plot_data, gof_metrics, y_limits) {
  ...
  ggplot(long_data, aes(x = dactive)) +
    ...
    geom_label(data = gof_metrics,
               aes(x = -Inf, y = Inf,
                   label = sprintf("RMSE(dist): %.2f", rmse_dactive)),
               hjust = -0.1, vjust = 1.1, size = ANNOTATION_SIZE,
               label.padding = unit(0.15, "lines"),
               label.r = unit(0, "lines"),
               fill = "white",
...
# Function to prepare summary data
prepare_summary_data <- function(data_gof) {
  data_gof %>%
    pivot_longer(cols = c(contains("dev.expl"), contains("rmse_lrmsf"), contains("rmse_dactive")),
...
# Updated plot_fits_vs_obs function

plot_fits_vs_obs <- function(profiles, data_gof, mcsa_id) {
  ...
  summary_data <- prepare_summary_data(data_gof)

  # Create summary plots
  p_best_model <- create_best_model_histogram(data_gof)
  p_dev_expl <- create_summary_plot(filter(summary_data, metric == "dev.expl"),
                                    "value", "Expl. Dev.", "Profile fit", x_limit = .85)
  p_rmse_lrmsf <- create_summary_plot(filter(summary_data, metric == "rmse_lrmsf"),
                                      "value", "RMSE(flex)", "Flexibility fit")
  p_rmse_dactive <- create_summary_plot(filter(summary_data, metric == "rmse_dactive"),
                                        "value", "RMSE(dist)", "Distance fit")
  ...
  summary_row <- plot_grid(p_best_model, p_dev_expl, p_rmse_lrmsf, p_rmse_dactive,
                           ncol = 4,
                           rel_widths = c(0.75, 0.75, 1, 1),
                           align = "h", axis = "tb", greedy = TRUE,
                           labels = c("(d)", "(e)", "(f)", "(g)"), label_size = 9)
  ...
  main_title <- ggdraw() +
    draw_label(
      "Observed vs. predicted structural divergence patterns",
      fontface = "plain",
      size = 12,
      color = "#4a4a4a"
    )

  # Create the main plot title
  main_plots_title <- ggdraw() +
    draw_label(
  ...
  # Create the summary row title
  summary_row_title <- ggdraw() +
    draw_label(
  ...
  final_plot <- plot_grid(
    ...
    summary_row,
    ncol = 1,
    rel_heights = c(0.3, 0.3, 3, 0.2, 0.2, 0.3, 1.5),  # Slightly increased heights for titles
    align = 'v'
  )
